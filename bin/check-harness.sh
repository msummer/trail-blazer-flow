...
if $gh_ready; then
  ...
  missing=""
  for l in plan-proposed plan-approved pr-open impl-blocked no-plan no-auto-approve test-ratchet multi-pr needs-human harness-stop; do
  ...
  if [ -z "$missing" ]; then
    ok "all 10 lifecycle labels exist"
  else
