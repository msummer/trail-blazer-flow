...
e. **As each implementer completes, run the issue-implementer skill's steps 2d–2e for that
   ...
   - **every git command takes `-C <worktree>`** — the verifier's diff is `git -C <worktree> diff
     ...
     REPO-LOCAL `.git/config` routes (since #290, a GLOBAL git config — `$GIT_CONFIG_GLOBAL`,
     `$XDG_CONFIG_HOME/git/config` or its default, `$HOME/.gitconfig` — is read identically for
     both the main checkout and the worktree, so a global `push.default`/`remote.<name>.push`
     route applies to either regardless of this per-checkout split), with the default-branch deny
     set unioned from both the worktree's checkout and the main checkout's own default (a sibling
     worktree shares the main checkout's default branch and config already, so only the current
     branch differs in practice). The worktree push below targets `claude/<number>-<slug>`,
