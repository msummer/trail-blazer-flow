...
orchestrator reads a filtered artifact instead of applying the rule from memory (#176) — the same
script also computes the approval-binding verdict described in "Approval provenance" above
(#174). The honest limit: this mechanical coverage is planner- and implementer-side only; the
verifier-side
