...
- 2026-09-24 (c): A verifier with a "probe at least one own mutant per round" mandate against a 500-line script
  ...
  where the only finding is a fresh survivor of that class), the orchestrator should stop and ask the maintainer
  to bound the round — "a new survivor with no false PASS / wrong exit / tracked-tree write / set mismatch is a
  Note and follow-up candidate, not a Finding" — rather than spend a third kickback.
